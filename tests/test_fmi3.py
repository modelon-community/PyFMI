...
class Test_FMUModelBase3:
    ...
    def test_set_enum_by_string_name(self):
        ...
        assert fmu.get("Enumeration_input") == [1]
    ...
    def test_set_enum_multiple_variables(self):
        fmu = _get_fmu(FMI3_REF_FMU_PATH / "Feedthrough.fmu", _connect_dll=True)
        fmu.set_enum(["Enumeration_input", "Enumeration_output"], ["Option 1", "Option 2"])
        assert fmu.get("Enumeration_input") == [1]

    def test_set_enum_string_with_value_reference_raises(self):
        fmu = _get_fmu(FMI3_REF_FMU_PATH / "Feedthrough.fmu", _connect_dll=True)
        vref = fmu.get_variable_valueref("Enumeration_input")
        with pytest.raises(FMUException) as exc_info:
            fmu.set_enum([vref], ["Option 1"])
        assert "variable name required for string lookup" in str(exc_info.value)

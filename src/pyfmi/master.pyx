...
cdef perform_do_step(list models, dict time_spent, FMIL.fmi2_import_t** model_addresses, double cur_time, double step_size, bool new_step, int setting=SERIAL):
    if setting == SERIAL:
    ...
    def jacobi_algorithm(self, double start_time, double final_time, object opts):
        ...
        if self.error_controlled:
            ...
            while tcur < final_time:
                ...
                self.set_input(tcur)
                #Take a full step
                perform_do_step(self.models, self.elapsed_time, self.fmu_adresses, tcur, step_size, new_step=False, setting=calling_setting)
                ...
                self.set_input(tcur)
                #Take a half step
                perform_do_step(self.models, self.elapsed_time, self.fmu_adresses, tcur, step_size/2.0, new_step=False, setting=calling_setting)
                ...
                perform_do_step(self.models, self.elapsed_time, self.fmu_adresses, tcur+step_size/2.0, step_size/2.0, new_step=False, setting=calling_setting)
            ...
            for tcur in np.arange(start_time, final_time, step_size):
                ...
                perform_do_step(self.models, self.elapsed_time, self.fmu_adresses, tcur, step_size, new_step=True, setting=calling_setting)
